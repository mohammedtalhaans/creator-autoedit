...
const PRESETS: Record<PresetName, Partial<PromptSettings>> = {
  lens: { fontSize: 38, lineHeight: 1.25, columnWidth: 86, windowHeight: 34, verticalPosition: 18, backgroundOpacity: .38, showBackground: true, textShadow: true, textOutline: false, readingLine: 16, textAlign: 'left', showReadingLine: true, dimSurrounding: true },
  minimal: { fontSize: 34, lineHeight: 1.25, columnWidth: 80, windowHeight: 30, verticalPosition: 15, backgroundOpacity: .18, showBackground: false, textShadow: true, textOutline: false, readingLine: 18, textAlign: 'left', showReadingLine: true, dimSurrounding: true },
  large: { fontSize: 56, lineHeight: 1.18, columnWidth: 92, windowHeight: 44, verticalPosition: 20, backgroundOpacity: .5, showBackground: true, textShadow: true, textOutline: false, readingLine: 16, textAlign: 'left', showReadingLine: true, dimSurrounding: false },
}
...
function PromptControlsFields({ settings, onChange, compact }: { settings: PromptSettings; onChange: (patch: Partial<PromptSettings>) => void; compact: boolean }) {
  ...
  const verticalPreset = settings.verticalPosition === 8 ? 'top' : settings.verticalPosition === 18 ? 'lens' : settings.verticalPosition === 34 ? 'middle' : settings.verticalPosition === 48 ? 'low' : ''
  return <>
    <div className="tp-prompt-control-grid">
      <div className="tp-prompt-font-field"><NativeSelect label="Font" value={settings.fontFamily} onChange={(event) => onChange({ fontFamily: event.target.value })}>
        <option>DM Sans Variable</option><option>Barlow Condensed</option><option>JetBrains Mono Variable</option><option>system-ui</option><option>Arial</option><option>Arial Black</option><option>Helvetica Neue</option><option>Avenir Next</option><option>Verdana</option><option>Trebuchet MS</option><option>Georgia</option><option>Times New Roman</option><option>Courier New</option><option>Impact</option>{settings.fontFamily.startsWith('Creator Local ·') && <option>{settings.fontFamily}</option>}
      ...
      <Slider label="Vertical position" value={settings.verticalPosition ?? 18} min={8} max={58} step={1} display={`${settings.verticalPosition ?? 18}% from top`} onChange={(verticalPosition) => onChange({ verticalPosition })}/>
    ...
    <div className="tp-prompt-align-row"><span className="tp-control-label">Screen position</span><Segmented label="Screen position" value={verticalPreset} items={[{ value: 'top', label: 'Top' }, { value: 'lens', label: 'Lens' }, { value: 'middle', label: 'Middle' }, { value: 'low', label: 'Low' }]} onChange={(position) => onChange({ verticalPosition: position === 'top' ? 8 : position === 'lens' ? 18 : position === 'middle' ? 34 : 48 })}/></div>
    <div className="tp-prompt-colour-grid"><label className="tp-color-field"><span>Text colour</span><input type="color" value={settings.textColor} onChange={(event) => onChange({ textColor: event.target.value })}/></label><label className="tp-color-field"><span>Prompt background</span><input type="color" value={settings.backgroundColor} disabled={settings.showBackground === false} onChange={(event) => onChange({ backgroundColor: event.target.value })}/></label><Slider label="Background opacity" value={settings.backgroundOpacity} min={0} max={1} step={.01} display={`${Math.round(settings.backgroundOpacity * 100)}%`} disabled={settings.showBackground === false} onChange={(backgroundOpacity) => onChange({ backgroundOpacity })}/><Slider label="Background blur" value={settings.backgroundBlur ?? 0} min={0} max={24} step={1} display={`${settings.backgroundBlur ?? 0}px`} disabled={settings.showBackground === false} onChange={(backgroundBlur) => onChange({ backgroundBlur })}/></div>
    <div className="tp-toggle-row tp-prompt-toggles"><Switch label="Show transcript" checked={settings.showPrompt !== false} onChange={(showPrompt) => onChange({ showPrompt })}/><Switch label="Show background" checked={settings.showBackground !== false} onChange={(showBackground) => onChange({ showBackground })}/><Switch label="Text shadow" checked={settings.textShadow !== false} onChange={(textShadow) => onChange({ textShadow })}/><Switch label="Text outline" checked={settings.textOutline === true} onChange={(textOutline) => onChange({ textOutline })}/><Switch label="Mirror prompt" checked={settings.mirror} onChange={(mirror) => onChange({ mirror })}/><Switch label="Dim surrounding text" checked={settings.dimSurrounding} onChange={(dimSurrounding) => onChange({ dimSurrounding })}/><Switch label="Show reading line" checked={settings.showReadingLine !== false} onChange={(showReadingLine) => onChange({ showReadingLine })}/></div>
