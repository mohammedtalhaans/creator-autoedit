...
export function HeroMachine() {
    ...
    return <div className="hero-machine" aria-label="Illustration: remove sound gaps, add captions, and choose a frame">
  ...
  <div className="machine-stage"><div className="monitor-grid"/><span className="monitor-corner tl"/><span className="monitor-corner br"/>
   ...
   <motion.div className="hero-frame" animate={{ width: active >= 3 ? '52%' : '86%' }} transition={{ type: 'spring', stiffness: 110, damping: 23 }}>
    <svg className="hero-subject" viewBox="0 0 420 370" aria-hidden="true"><defs><linearGradient id="shirt" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#8A8A8A"/><stop offset="1" stopColor="#363636"/></linearGradient><linearGradient id="face" x1="0" y1="0" x2="1" y2=".8"><stop stopColor="#A6A6A6"/><stop offset="1" stopColor="#555555"/></linearGradient><radialGradient id="wall"><stop stopColor="#434343"/><stop offset="1" stopColor="#202020"/></radialGradient><pattern id="micro" width="4" height="4" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r=".45" fill="#fff" opacity=".07"/></pattern></defs><rect width="420" height="370" fill="url(#wall)"/><path d="M30 0V370M74 0V370M346 0V370M389 0V370" stroke="#fff" opacity=".045"/><circle cx="310" cy="86" r="52" fill="#8D8D8D" opacity=".10"/><ellipse cx="215" cy="348" rx="153" ry="16" fill="#090909" opacity=".4"/><path d="M72 370L90 255Q106 224 177 209H251Q320 223 337 255L354 370Z" fill="url(#shirt)"/><path d="M179 186V222Q214 248 250 219V183" fill="url(#face)"/><path d="M166 91Q163 44 214 44Q268 43 271 96L262 161Q251 196 214 205Q174 194 165 158Z" fill="url(#face)"/><path d="M165 116L158 99Q157 35 214 32Q281 35 271 115L257 85Q235 86 222 70Q192 92 172 86Z" fill="#232323"/><path d="M180 123Q190 117 199 123M231 123Q241 117 250 123" stroke="#353535" strokeWidth="4" fill="none"/><path d="M214 126L210 149L220 151" stroke="#5F5F5F" strokeWidth="3" fill="none"/><path d="M198 170Q216 180 235 167" stroke="#3E3E3E" strokeWidth="3" fill="none"/><path d="M176 233L154 370M251 233L278 370" stroke="#A4A4A4" opacity=".2"/><rect width="420" height="370" fill="url(#micro)"/></svg>
   ...
   <div className="hero-meter"><span className="mono">AUDIO</span>{Array.from({ length: 18 }, (_, i) => <motion.i key={i} animate={{ opacity: active >= 2 ? (i < 13 ? 1 : .16) : (i < 9 + (step % 2) * 7 ? 1 : .16) }} className={i > 14 ? 'peak' : ''}/>)}<span className="mono">−12</span></div>
  ...
  <div className="machine-timeline"><div className="timeline-heading"><span className="mono">{active ? 'THE GOOD PARTS' : 'YOUR ONE TAKE'}</span><span className="duration"><NumberTicker value={active ? 68 : 94} format={n => `01:${Math.max(0, Math.round(n - 60)).toString().padStart(2, '0')}`}/><span className="mono"> / 01:34</span></span></div>
   ...
   <div className="machine-modules">{[{ icon: Scissors, label: 'Autocut' }, { icon: Captions, label: 'Captions' }, { icon: AudioLines, label: 'Audio' }, { icon: ScanFace, label: 'Frame' }].map((item, i) => <div key={item.label} className={active > i ? 'is-lit' : ''}><item.icon size={12}/>{item.label}{active > i && <Check size={11}/>}</div>)}</div>
  </div>
