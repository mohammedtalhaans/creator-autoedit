...
async function copyRuntime(pkg,folder,destination,pattern) {
 ...
 for(const name of names) {const source=join(pkg.dir,folder,name),dest=join(target,name);await copyFile(source,dest);const bytes=await readFile(dest);manifest.assets.push({path:relative(join(root,'public'),dest).replaceAll('\\','/'),bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex'),package:pkg.p.name,version:pkg.p.version});}
}
...
await copyRuntime(vision,'wasm','vision',/^vision_[\w.-]+\.(wasm|js|mjs)$/);
