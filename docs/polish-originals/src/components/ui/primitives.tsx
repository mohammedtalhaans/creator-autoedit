...
export function Segmented<T extends string>({ label, value, items, onChange }: {
    ...
    return <div className="segmented" role="group" aria-label={label}>{items.map(item => <button key={item.value} type="button" className={cn(value === item.value && 'selected')} aria-pressed={value === item.value} disabled={item.disabled} onClick={() => onChange(item.value)}>{item.label}</button>)}</div>;
}
