...
export function Editor({ project, onAbout, onNew }: {
    ...
    const [compare, setCompare] = useState<'raw' | 'edited'>('edited'), [holding, setHolding] = useState(false), [voiceRaw, setVoiceRaw] = useState(false), [guides, setGuides] = useState(false), [fullscreen, setFullscreen] = useState(false), [sheet, setSheet] = useState(false), [exportOpen, setExportOpen] = useState(false), [info, setInfo] = useState(false);
    const raw = compare === 'raw' || holding;
    const cuts = useMemo(() => decisionsFor(project.pauses, project.cutPreset, project.sensitivity, project.overrides), [project.pauses, project.cutPreset, project.sensitivity, project.overrides]);
    ...
    useEffect(() => {
        const keyboard = (e: KeyboardEvent) => {
            ...
            if (e.key === 'ArrowRight') {
                e.preventDefault();
                raw ? player.current?.seekSource(transport.time + 3) : player.current?.seek(outputTime + 3);
            }
            if (e.key === 'ArrowLeft') {
                e.preventDefault();
                raw ? player.current?.seekSource(transport.time - 3) : player.current?.seek(outputTime - 3);
  ...
  <main className="workspace"><GlowSurface className={cn('player-panel', fullscreen && 'fullscreen-preview')}><div className="player-toolbar"><span className="mono">THE MONITOR <span className="slash">/</span> {raw ? 'ORIGINAL TAKE' : 'YOUR EDIT'}</span><div><IconButton label={guides ? 'Hide social safe zones' : 'Show social safe zones'} onClick={() => setGuides(!guides)} aria-pressed={guides}><Scan size={15}/></IconButton><IconButton label={fullscreen ? 'Exit immersive preview' : 'Expand preview'} onClick={() => setFullscreen(!fullscreen)}>{fullscreen ? <Minimize2 size={15}/> : <Maximize2 size={15}/>}</IconButton></div></div>
   ...
   <div className="player-bottom"><div className="preview-comparison"><Segmented label="Compare original and edited video" value={compare} onChange={setCompare} items={[{ value: 'raw', label: 'Raw' }, { value: 'edited', label: 'Edited' }]}/><button className="hold-compare" onPointerDown={e => { e.preventDefault(); e.currentTarget.setPointerCapture(e.pointerId); setHolding(true); }} onPointerUp={() => setHolding(false)} onPointerCancel={() => setHolding(false)} onLostPointerCapture={() => setHolding(false)} onKeyDown={e => {
    ...
    <div className="transport"><div className="transport-buttons"><IconButton label="Back to start" onClick={() => raw ? player.current?.seekSource(0) : player.current?.seek(0)}><SkipBack size={16}/></IconButton><button className="play-button" onClick={() => player.current?.toggle()} aria-label={transport.playing ? 'Pause video' : 'Play video'}>{transport.playing ? <Pause size={19} fill="currentColor"/> : <Play size={19} fill="currentColor"/>}</button></div><div className="timecode"><span className="mono">{timecode(raw ? transport.time : outputTime, true)}</span><span className="mono"> / {timecode(raw ? project.metadata.duration : map.outputDuration)}</span></div><span className="preview-status mono">{raw ? 'RAW TAKE' : project.audio.enabled && !voiceRaw && state.audioUrl ? 'VOICE ENHANCED' : 'ORIGINAL VOICE'}</span></div>
   </div></GlowSurface>
  ...
  {mobile && <><ModuleRail className="mobile-rail" active={state.module} onChange={m => { studio.module(m); setSheet(true); }}/><Modal open={sheet} onOpenChange={setSheet} title={modules.find(m => m.id === state.module)!.label} description="Changes are live in your preview." sheet><Properties {...props}/><Button className="sheet-done" variant="primary" onClick={() => setSheet(false)}>Back to preview<ArrowRight size={15}/></Button></Modal></>}
